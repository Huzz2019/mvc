    ...
    private void initHandlerMapping() {
        ...
        for(Map.Entry<String,Object> entry: ioc.entrySet()) {
            ...
            for (int i = 0; i < methods.length; i++) {
                ...
                if(method.isAnnotationPresent(HuzzSecurity.class)){
                    HuzzSecurity huzzSecurity = method.getAnnotation(HuzzSecurity.class);
                    nameList = Arrays.asList(huzzSecurity.value());
                }

                // 把method所有信息及url封装为一个Handler
                Handler handler = new Handler(entry.getValue(),method, Pattern.compile(url),nameList);
...
//        method.invoke() //
        ...
        List<String> peopleWithAuthorityList = handler.getPeopleWithAuthorityList();
        String name  = (String) req.getParameter("name");
        req.setAttribute("name",name);
        if(peopleWithAuthorityList.contains(name)){
